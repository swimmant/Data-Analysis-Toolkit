...
def calculate_prec_recall(result,label,iou_thresh):
    ...
    return prec,rec
def calculate_VOC_mAP(result,classnames,iou_thresh=0.5,use_07_metric=False):
    '''
    INPUTS:
    ...
    if len(aps)>0:
        ...
        mAP = 0.
    return mAP,all_aps,prs
def draw_ap(prec,rec):
    ...
    plt.show()
# 仅作为方法使用例子，不代表真实数据
result = pickle.load(open("tt100k_2021/test_result.pkl","rb"))
classnames = ['pl80', 'w9', 'p6', 'ph4.2', 'i8', 'w14', 'w33', 'pa13', 'im', 'w58', 'pl90', 'il70', 'p5', 'pm55', 'pl60']
mAP,aps,prs = calculate_VOC_mAP(result,classnames)
print(mAP)
draw_ap(*prs["pl80"])
# Output： 0.6583333333333334